"""
Adding-problem comparison: vanilla RNN vs scratch GRU.

Trains both models from scratch on the adding problem at a chosen sequence
length and reports a side-by-side learning curve. The whole point is to see
the vanilla RNN plateau near the constant-1.0 baseline (MSE ~0.167) while
the GRU drops well below it.
...
def plot_curves(results: list[dict], seq_len: int, out_path: str) -> None:
    ...
    for r in results:
        plt.plot(r["steps"], r["losses"], label=f"{r['kind'].upper()} ({r['params']:,} params)")
    plt.axhline(
        NAIVE_BASELINE_MSE,
        color="gray",
        linestyle="--",
...
def main():
    ...
    for kind in args.models:
        ...
        print(
            ...
            f"params={r['params']:,}, final MSE={r['final_loss']:.4f}\n"
        )
        results.append(r)

    print("\n=== summary ===")
    print(f"{'model':<6} {'params':>8} {'final MSE':>11} {'time(s)':>9}")
    for r in results:
        print(f"{r['kind']:<6} {r['params']:>8,} {r['final_loss']:>11.4f} {r['elapsed_s']:>9.1f}")
    print(f"\nbeating MSE {NAIVE_BASELINE_MSE:.3f} = the model has learned something.")
